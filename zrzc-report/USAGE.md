...
`zrzc-report` 是一个自动生成「影响范围评估报告」的 Claude Code / Cursor Skill。给定一个需求 PRD 和对应的代码仓库，它会：
...
`zrzc-report/` 目录即为本 Skill。

### 方式二：放到 Skill 目录

将 `zrzc-report/` 目录放到你的工具 Skill 目录下：

- Claude Code：`~/.claude/skills/zrzc-report/`
- Cursor：`~/.cursor/skills/zrzc-report/`

```bash
# 示例：克隆后拷贝到 Claude Code 的 skills 目录
git clone git@github.com:zhaomengjun-vast/qa-skills.git
cp -r qa-skills/zrzc-report ~/.claude/skills/zrzc-report
```
...
在 Claude Code / Cursor 中，用自然语言触发 Skill（触发词：**影响范围评估 / ZRZC / 变更分析报告**），并提供三项必需输入：
...
- **本地文件**：`zrzc-report/output/{需求名}-影响范围评估报告.md`
